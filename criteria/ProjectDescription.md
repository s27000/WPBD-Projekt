...
Deliverables
•	Python script that: 
o	Reads three CSV files.
o	Creates PostgreSQL tables dynamically.
o	Inserts data into PostgreSQL.
...
•	Enabling JSON serialization for messages sent to Kafka.
Deliverables
•	Configured Debezium service in docker-compose.yml.
•	PostgreSQL configured with logical replication enabled.
•	Debezium configured to use JSON format for messages.
•	Verified that changes in PostgreSQL are captured by Debezium and sent to Kafka in JSON format.
________________________________________
Task 3: Kafka Setup & Streaming Events in AVRO Format
Topics
•	Setting up a Kafka cluster in Docker.
•	Configuring Kafka topics to use JSON format.
•	Setting up Schema Registry for JSON message serialization.
•	Writing a Kafka consumer to validate incoming messages.
Deliverables
•	Running Kafka cluster in docker-compose.yml.
•	Kafka configured with JSON serialization.
•	Schema Registry set up to manage JSON schemas.
•	Kafka consumer script that reads JSON messages and decodes them.
________________________________________
...
•	Deserializing JSON messages from Kafka in Spark.
•	Performing basic transformations on streamed data.
Deliverables
•	Spark container added to docker-compose.yml.
•	Spark job that reads JSON messages from Kafka.
